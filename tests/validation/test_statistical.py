...
class TestSimulatorStatistics:
    ...
    def test_mean_reversion(self):
        ...
        # Long-run mean should be near zero (no drift)
        final_mean = np.mean(paths[:, -1])
        assert abs(final_mean) < 2.0  # reasonable bound
    ...
    def test_volatility_clustering(self):
        ...
        # Squared returns should be positively autocorrelated
        sq = returns ** 2
    ...
    def test_student_t_tails(self):
        ...
        # Kurtosis should be > 3 (normal kurtosis)
...
class TestVaRValidation:
    ...
    def test_historical_var_coverage(self, mock_returns):
        ...
        # Should be approximately 5% with some tolerance
    ...
    def test_var_monotonic_in_confidence(self, mock_returns):
        """Higher confidence should give higher VaR (in absolute value for upper tail)."""
        from src.risk.var_engine import VaREngine
...
class TestModelSelection:
    ...
    def test_known_dgp_recovery(self):
        """When DGP is GARCH(1,1), GARCH should win over EWMA in AIC."""
        from src.core.simulator import SpreadSimulator
        from src.models.garch import GARCHModel
        from src.models.ewma import EWMAModel

        # Generate data from GARCH(1,1) with strong persistence
        ...
        # GARCH should have better (lower) AIC since it matches the DGP
        if garch.result.aic is not None:
            assert garch.result.aic < ewma.result.aic + 50, \
                "GARCH should fit GARCH-DGP better than EWMA"
